...
LoadingState.create = function () {
    //newCoins.push(levelCache)
    //window.levelCache = this.game.cache.getJSON("level:0");
...
function logCurrentState (game, coin) {
    levelCache = game.cache.getJSON("level:0");

    for(value of levelCache.coins){
        if(coin.x === value.x){
            levelCache.coins.splice(levelCache.coins.indexOf(value), 1)
            console.log(value)
        }
    }

    console.log(levelCache.coins)
